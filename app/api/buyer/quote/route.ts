...
export async function GET(request: NextRequest) {
  ...
  const reqMap = new Map((reqResult.data ?? []).map(x => [x.id, x])); const supplyMap = new Map((supplyResult.data ?? []).map(x => [x.id, x]));
...
export async function POST(request: NextRequest) {
  ...
  const requirementId = String(body?.requirementId || ''); const supplyListingId = String(body?.supplyListingId || '');
  ...
  const { data: requirement } = await c.admin.from('farmplug_buyer_requirements').select('id,buyer_name,crop,quantity_kg,quality,location,delivery_days,status,created_by').eq('id', requirementId).maybeSingle();
  const { data: supply } = await c.admin.from('farmplug_supply_listings').select('id,farmer_name,crop,quantity_kg,quality,location,available_until,status,created_by').eq('id', supplyListingId).maybeSingle();
  ...
  if (requirement.created_by && requirement.created_by !== user.id) return NextResponse.json({ error: 'You can only request quotes for your own buyer requirements.' }, { status: 403 });
  const { data: existing } = await c.admin.from('farmplug_quote_requests').select('id,status').eq('requirement_id', requirementId).eq('supply_listing_id', supplyListingId).eq('buyer_id', user.id).maybeSingle();
  if (existing) return NextResponse.json({ request: existing, alreadyRequested: true });
  const { data, error } = await c.admin.from('farmplug_quote_requests').insert({ requirement_id: requirementId, supply_listing_id: supplyListingId, buyer_id: user.id, message }).select('id,requirement_id,supply_listing_id,status,created_at').single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
