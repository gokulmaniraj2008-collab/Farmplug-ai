...
type Quote = { id: string; requirement_id: string; supply_listing_id: string; status: string; created_at: string; supply: Supply | null; requirement: Requirement | null };
...
export default function BuyerCenter() {
  ...
  async function loadQuotes() {
    ...
    if (!session) return;
    const r = await fetch("/api/buyer/quote", { headers: { Authorization: `Bearer ${session.access_token}` }, cache: "no-store" });
    if (!r.ok) return;
    const data = await r.json();
    setQuotes(data.quotes ?? []);
    setRequested((data.quotes ?? []).map((q: Quote) => q.supply_listing_id));
  }
  ...
  useEffect(() => {
    Promise.all([fetch("/api/buyer/requirements", { cache: "no-store" }).then(r => r.json()), fetch("/api/buyer/supply", { cache: "no-store" }).then(r => r.json())])
      .then(async ([req, sup]) => { setRequirements(req.requirements ?? []); setSupply(sup.supply ?? []); await loadQuotes(); })
  ...
  async function requestQuote(item: Supply) {
    ...
    setQuotes(x => data.alreadyRequested ? x : [{ ...data.request, requirement, supply: item }, ...x]);
    setMessage(data.alreadyRequested ? `A quote request for ${item.farmer_name} already exists.` : `Quote request sent to ${item.farmer_name}.`);
  }
  ...
  return (
    <main className="buyerPage">
      ...
      <section className="buyerWrap">
        <div className="buyerStats"><div><span>OPEN REQUIREMENTS</span><strong>{requirements.length}</strong></div><div><span>MATCHED SUPPLY</span><strong>{matchedQty.toLocaleString()} kg</strong></div><div><span>HIGH MATCHES</span><strong>{matched.length}</strong></div></div>
        ...
        <section className="buyerCard"><div className="sectionTitle"><div><span className="eyebrow">3 · REQUEST TRACKING</span><h2>My quote requests</h2></div><CheckCircle2 size={22} /></div>
          {!supabase ? <p className="muted">Sign-in is required to track quote requests.</p> : quotes.length === 0 ? <p className="muted">No quote requests yet. Request a quote from a matched supply listing above.</p> : <div className="quoteList">{quotes.map(q => <article className="quoteRow" key={q.id}><div><b>{q.supply?.farmer_name ?? "Farmer / FPO"}</b><span>{q.supply?.crop ?? q.requirement?.crop ?? "Produce"} · {Number(q.supply?.quantity_kg ?? 0).toLocaleString()} kg · {q.supply?.location ?? "—"}</span><small>{new Date(q.created_at).toLocaleString()}</small></div><span className={`quoteStatus ${q.status}`}>{q.status}</span></article>)}</div>}
        </section>
        <section className="buyerGrid"><div className="buyerCard"><span className="eyebrow">4 · BULK OPPORTUNITY</span><h2>Consolidate the requirement</h2><div className="bulk">{matched.slice(0,2).map((item, i) => <div key={item.id}><strong>{Number(item.quantity_kg).toLocaleString()} kg</strong><span>{item.farmer_name}</span>{i === 0 && matched.length > 1 ? <em>+</em> : null}</div>)}<div className="total"><strong>{matchedQty.toLocaleString()} kg</strong><span>Matched total</span></div></div><p className="muted">FarmPlug aligns fragmented FPO supply into a buyer-ready lot. Live values come from the FarmPlug marketplace database.</p></div><div className="buyerCard"><span className="eyebrow">5 · DELIVERY PLAN</span><h2>Coordinate the last mile</h2><div className="delivery"><Truck size={25} /><div><b>Consolidated delivery</b><span>{matched.map(x => x.location).slice(0,3).join(" + ")} → {requirement?.location ?? "Buyer"}</span><small>OpenStreetMap-compatible architecture · demo route</small></div></div><div className="notice">No real-time vehicle tracking or live routing is claimed.</div></div></section>
