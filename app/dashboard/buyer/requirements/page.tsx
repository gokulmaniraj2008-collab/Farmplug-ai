...
export default function BuyerRequirementsPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      ...
      .select("id, crop, quantity_kg, quality, location, delivery_days, status")
  ...
  return (
    <div className="mx-auto max-w-2xl p-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <Link href="/dashboard/buyer/requirements/new" className="rounded-md bg-[#1B4332] px-4 py-2 text-sm font-medium text-white">
          + Create Requirement
        </Link>
      </div>
        ...
        <div className="mt-4 space-y-2">
          {requirements.map((req) => (
            <div key={req.id} className="rounded-md border border-gray-200 p-3">
              <div className="flex items-center justify-between gap-3">
                <p className="text-sm font-medium text-[#17211B]">{req.crop} · {req.quantity_kg} kg · {req.quality}</p>
                <StatusBadge status={req.status} />
              </div>
              <p className="mt-1 text-xs text-[#5F6B63]">{req.location} · {req.delivery_days} day delivery window</p>
            </div>
          ))}
        </div>
      )}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`rounded-full px-2 py-1 text-xs font-medium ${styles[status] ?? styles.open}`}>{status}</span>;
}
