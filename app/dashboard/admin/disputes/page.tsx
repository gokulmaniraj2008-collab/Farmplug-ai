...
export default function AdminDisputesPage() {
  ...
  async function resolve(id: string, status: DisputeStatus) {
    setBusyId(id);
    setError(null);
    const supabase = createClient();
    const { error: rpcError } = await supabase.rpc("resolve_dispute", {
      p_dispute_id: id,
      p_new_status: status,
      p_resolution: resolutionDrafts[id] ?? null,
    });
  ...
  return (
    <div className="mx-auto max-w-3xl p-6">
      ...
      {disputes.length === 0 && (
        <p className="mt-4 text-sm text-gray-500">No disputes on file.</p>
      )}
          ...
          <li key={d.id} className="rounded-md border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-500">
                Order #{d.order_id.slice(0, 8)} — {new Date(d.created_at).toLocaleString()}
              </span>
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_STYLE[d.status]}`}>
                {d.status}
              </span>
            </div>
            <p className="mt-2 text-sm text-gray-900">{d.reason}</p>
            {d.resolution && (
              <p className="mt-2 text-sm text-gray-600">Resolution: {d.resolution}</p>
            )}
