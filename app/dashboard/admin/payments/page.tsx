...
export default function AdminPaymentsPage() {
  ...
  async function refund(orderId: string) {
    ...
    setError(null);
    const supabase = createClient();
    const { error: rpcError } = await supabase.rpc("simulate_refund_payment", { p_order_id: orderId });
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      ...
      <p className="mt-1 text-sm text-amber-600">All amounts are simulated. No real payment gateway is connected — nothing here moves real money.</p>
      {error && <p className="mt-2 text-sm text-red-600">{error}</p>}
      <div className="mt-4 flex flex-wrap gap-2 text-xs text-gray-600">
        {Object.entries(totalsByStatus).map(([status, count]) => <span key={status} className="rounded-full bg-gray-100 px-3 py-1">{status}: {count}</span>)}
      </div>
      <ul className="mt-6 space-y-3">
        {payments.length === 0 && <p className="text-sm text-gray-500">No payment records yet.</p>}
        {payments.map((p) => (
          <li key={p.order_id} className="rounded-md border border-gray-200 p-4">
            <div className="flex items-center justify-between">
              <span className="text-sm text-gray-900">Order #{p.order_id.slice(0, 8)} — {p.currency} {p.amount.toFixed(2)}</span>
              <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_STYLE[p.status]}`}>{p.status} (simulated)</span>
            </div>
            <p className="mt-1 text-xs text-gray-500">Recorded {new Date(p.created_at).toLocaleString()}</p>
            {(p.status === "authorized" || p.status === "paid" || p.status === "released") && (
              <button type="button" onClick={() => refund(p.order_id)} disabled={busyOrderId === p.order_id} className="mt-3 rounded-md border border-red-300 px-3 py-1.5 text-xs font-medium text-red-700 disabled:opacity-50">
