...
export default function AddCropPage() {
  ...
  async function handleSubmit() {
    ...
    if (farmError || !farm) {
      setError("Set up your farm profile first, then add crops.");
      setSaving(false);
      return;
    }
    ...
    const { error: insertError } = await supabase.from("crops").insert({
      ...
      crop_calendar_notes: form.crop_calendar_notes || null,
    });
  ...
  return (
    <div className="mx-auto max-w-md p-6">
      <h1 className="text-xl font-semibold text-[#17211B]">Add Crop</h1>
      ...
      <div className="mt-4 space-y-3">
        ...
        <input
          ...
          className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
        />
        <label className="block text-xs text-[#5F6B63]">Expected harvest date</label>
        <input
          type="date"
        ...
        />
        <textarea
          placeholder="Crop calendar notes (optional)"
          value={form.crop_calendar_notes}
          onChange={(e) => setForm({ ...form, crop_calendar_notes: e.target.value })}
          className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
          rows={3}
        />
      </div>
