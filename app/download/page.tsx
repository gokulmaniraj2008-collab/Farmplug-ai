...
export default function DownloadPage() {
  ...
  async function installApp() {
    if (!deferredPrompt) {
      window.alert(
        'To install FarmPlug on Android: open this page in Chrome, tap ⋮, then choose “Install app” or “Add to Home screen”.'
      );
      return;
    }
    ...
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '24px', background: 'linear-gradient(145deg,#f4fbf1,#ffffff)' }}>
      <section style={{ width: '100%', maxWidth: 560, textAlign: 'center', padding: '32px', borderRadius: 28, background: '#fff', boxShadow: '0 20px 60px rgba(25,80,30,.12)', border: '1px solid #dcebd8' }}>
        ...
        <p style={{ lineHeight: 1.6, color: '#5b6d5e' }}>
          Use FarmPlug like an Android app without maintaining a separate Flutter client. List produce, run AI decisions, and track farm-to-market orders from one source of truth.
        </p>

        <button
          onClick={installApp}
          disabled={installed}
          style={{ width: '100%', padding: '15px 20px', border: 0, borderRadius: 14, background: installed ? '#64756a' : '#166534', color: '#fff', fontWeight: 800, fontSize: 16, marginTop: 22, cursor: installed ? 'default' : 'pointer' }}
        >
          {installed ? 'FarmPlug Installed ✓' : deferredPrompt ? 'Install FarmPlug on Android' : 'Install FarmPlug'}
        </button>
        ...
        <div style={{ textAlign: 'left', marginTop: 22, padding: 16, borderRadius: 16, background: '#f7fbf7', border: '1px solid #e3ece5' }}>
          ...
          <p style={{ fontSize: 13, lineHeight: 1.5, color: '#5b6d5e', margin: '7px 0 0' }}>
            If Chrome does not show the automatic install prompt, tap <b>⋮</b> in Chrome and choose <b>Install app</b> or <b>Add to Home screen</b>.
          </p>
        </div>
