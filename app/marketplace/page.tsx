...
import { buildMarketplaceSearchFilter } from "@/lib/marketplace-search";
...
export default async function MarketplacePage({ searchParams }: MarketplacePageProps) {
  ...
  const searchFilter = buildMarketplaceSearchFilter(query);
  if (searchFilter) {
    listingsQuery = listingsQuery.or(searchFilter);
  }
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <form action="/marketplace" method="get" className="mt-6 flex flex-col gap-2 sm:flex-row">
        ...
        <input id="marketplace-search" name="q" defaultValue={query} placeholder="Search crop, location or quality..." autoComplete="off" className="min-h-11 flex-1 rounded-lg border border-gray-300 bg-white px-4 text-sm text-[#17211B] outline-none ring-[#1B4332] focus:ring-2" />
      ...
      {error ? (
        <div className="mt-8 rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-800">Marketplace listings could not be loaded right now. Please try again.</div>
      ) : !data?.length ? (
        <div className="mt-10 rounded-2xl border border-dashed border-gray-300 p-10 text-center text-sm text-[#5F6B63]">{query ? `No published listings matched “${query}”.` : "No published listings are available yet."}</div>
      ) : (
        <>
          <p className="mt-8 text-xs font-semibold uppercase tracking-wide text-[#6B756D]">{query ? `Results for “${query}”` : "Latest published listings"} · {data.length} shown</p>
