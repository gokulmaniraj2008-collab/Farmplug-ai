...
const items = [
  ...
  { href: '/crop-health', label: 'Crop Health', icon: FlaskConical },
  { href: '/farm-intelligence', label: 'Farm Intelligence', icon: Map },
  ...
  { href: '/portals', label: 'Portals', icon: LayoutGrid },
...
export default function Sidebar() {
    ...
    <nav className="sideNav" aria-label="FarmPlug navigation">
      {items.map(({ href, label, icon: Icon }) => (
        <Link
          ...
          className={isActive(href) ? 'sideItem active' : 'sideItem'}
        >
          <Icon size={19} strokeWidth={isActive(href) ? 2.5 : 2} />
          <span>{label}</span>
        </Link>
      ))}
    </nav>
  ...
  return (
    <>
      <button className="desktopMenuButton" onClick={() => setOpen(true)} aria-label="Open navigation">
        <Menu size={22} />
      </button>
      ...
      <aside className={open ? 'desktopSidebar open' : 'desktopSidebar'} aria-label="Desktop navigation">
        <div className="sideBrand">
          ...
          <div>
            <strong>FarmPlug AI</strong>
            <small>Farm intelligence → market</small>
          </div>
          <button className="sideClose" onClick={() => setOpen(false)} aria-label="Close navigation">
            <X size={21} />
          </button>
        </div>
        {nav()}
        <div className="sideFooter">
          SIH 2026 · PS 26033
          <br />
          <span>Professional platform prototype</span>
        </div>
      </aside>
      <button className="mobileMenuButton" onClick={() => setOpen(true)} aria-label="Open navigation">
        <Menu size={22} />
      </button>
      ...
      <nav className="standaloneBottomNav" aria-label="App navigation">
        {appItems.map(({ href, label, icon: Icon }) => (
          <Link
            key={href}
            href={href}
            className={isActive(href) ? 'bottomItem active' : 'bottomItem'}
          >
