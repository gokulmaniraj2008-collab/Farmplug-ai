...
type Result = { demandLevel: string; demandScore: number; sellingWindowDays: string; buyerMatches: number; bulkOpportunity: string; freshnessRisk: string; logisticsPriority: string; reasons: string[]; prototype: true };
...
export default function DecisionCenter() {
  ...
  return <main className="pageShell"><header className="mobilePageHead"><Link href="/" className="back"><ArrowLeft size={18}/></Link><div><b>FarmPlug AI</b><span>Decision Center</span></div><Sparkles size={20}/></header>
    ...
    {result && <section className="pageResults"><div className="status"><CheckCircle2 size={15}/> Prototype analysis ready</div><div className="miniGrid"><div><small>DEMAND OUTLOOK</small><strong>{result.demandLevel} · {result.demandScore}/100</strong></div><div><small>SELLING WINDOW</small><strong>{result.sellingWindowDays}</strong></div><div><small>BUYER MATCHES</small><strong>{result.buyerMatches} potential</strong></div><div><small>BULK OPPORTUNITY</small><strong>{result.bulkOpportunity}</strong></div><div><small>FRESHNESS RISK</small><strong>{result.freshnessRisk}</strong></div><div><small>LOGISTICS PRIORITY</small><strong>{result.logisticsPriority}</strong></div></div><div className="notice"><b>Why this result</b><ul>{result.reasons.map((reason, i) => <li key={i}>{reason}</li>)}</ul></div></section>}
  </main>;
}
