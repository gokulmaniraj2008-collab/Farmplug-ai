...
// Protects role-based dashboards and onboarding routes.
...
export async function middleware(request: NextRequest) {
  ...
  if (user && (isDashboardRoute || isOnboardingRoute)) {
    ...
    if (!profile.profile_complete) {
      if (path !== "/onboarding/complete-profile") {
    ...
    if (isOnboardingRoute) {
      return NextResponse.redirect(
        new URL(ROLE_PREFIX[profile.role] ?? "/", request.url)
      );
    }

    if (isDashboardRoute) {
      const allowedPrefix = ROLE_PREFIX[profile.role];
