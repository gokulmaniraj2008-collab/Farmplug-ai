...
begin
  ...
  if not (
    (old_status='quote_pending' and p_next_status in ('quote_accepted','cancelled')) or
    ...
    (old_status='delivered' and p_next_status='completed')
  ) then raise exception 'Invalid order transition: % -> %', old_status, p_next_status; end if;
  if not is_admin_actor then
    if p_next_status in ('quote_accepted','order_confirmed') and not is_buyer_actor then raise exception 'Not authorized'; end if;
    ...
    if p_next_status='cancelled' and not (is_buyer_actor or is_farmer_actor or is_fpo_actor) then raise exception 'Not authorized'; end if;
  end if;
