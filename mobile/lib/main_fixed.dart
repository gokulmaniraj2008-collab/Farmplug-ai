...
class _FarmerHomeState extends State<FarmerHome> {
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('FarmPlug AI')),
    body: pages[tab],
...
class HomeTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    ...
    const Card(child: ListTile(leading: CircleAvatar(child: Icon(Icons.auto_awesome)), title: Text('AI Decision Center'), subtitle: Text('Demand, selling window, buyer fit and logistics.'))),
    const Card(child: ListTile(leading: CircleAvatar(child: Icon(Icons.storefront)), title: Text('Sell your produce'), subtitle: Text('Prepare produce for buyers and FPO aggregation.'))),
    const Card(child: ListTile(leading: CircleAvatar(child: Icon(Icons.local_shipping)), title: Text('Track fulfilment'), subtitle: Text('Follow collection, order and delivery progress.'))),
...
class ProduceTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(16), children: [
    ...
    const SizedBox(height: 16),
    FilledButton.icon(onPressed: () => showDialog(context: context, builder: (_) => AlertDialog(title: const Text('Add Produce'), content: const Text('Use the FarmPlug web dashboard to publish a real authenticated listing.'), actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('OK'))])), icon: const Icon(Icons.add), label: const Text('Add Produce')),
  ]);
}
...
class _DecisionTabState extends State<DecisionTab> {
  ...
  Future<void> analyze() async {
    ...
    } catch (_) { setState(() => result = 'Network unavailable. The app remains available offline.'); }
